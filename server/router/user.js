...
router.get('/searchUser',async (req,res)=>{
    ...
    res.status(200).json(search.userExists);
    else
    res.status(200).json(search.userExists);
...
router.get('/getUser',async (req,res)=>{
    ...
    if(readUserData.success)
    res.status(200).json(readUserData.data);
    else
    res.status(404).json(readUserData.message);
    ...
    try{
        ...
        if(userExists == null){
            ...
            const registerUser=new User({
                userId:allUsers.length == 0 ? 0 : allUsers[allUsers.length - 1].userId+1,
                contactGroups:[],
                name,
                email,
                pass
