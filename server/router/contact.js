...
class ContactsData{
    ...
    async create(
        ...
        try{
            ...
            const contactExists=allContacts.find(currentElement=>{
                return (currentElement.contactNumber == contactNumber || currentElement.contactEmail === contactEmail)
            });
    ...
    async update(
        {userId,id},
        {contactName,contactNumber,contactEmail,contactType},image
        ){
        try{
...
router.get('/allContacts/',async (req,res)=>{
    ...
    if(allContacts.success)
        ...
        res.status(404).json(allContacts);
});
...
router.get('/starContact/',async (req,res)=>{
    ...
    if(starContact.success)
        res.status(200).json(starContact);
    else
        res.status(404).json(starContact);
});

// server images to the front end
router.get('/images/',async (req,res)=>{
     res.sendFile(`C:/Users/LENOVO/Desktop/project1/server/images/${req.query.imgName}`)
});

// create a new contact for the user with the passed userId 
// and the passed contact data
router.post('/createContact/',upload.single('contactImage'),async (req,res)=>{
    const contact=new ContactsData;
    const createContact=await contact.create(req.query,req.body,req.file);
    
    if(createContact.success || createContact.message === 'A contact already exists with the given details')
        res.status(200).json(createContact);
    else
        res.status(404).json(createContact);
});


// update an existing contact  with the newly passed contact details
// for the user with the passed userId
router.patch('/updateContact/',upload.single('contactImage'),async (req,res)=>{
    const contact=new ContactsData;
    const updateContact=await contact.update(req.query,req.body,req.file);
    if(updateContact.success)
        res.status(200).json(updateContact);
    else
        res.status(404).json(updateContact);
});
